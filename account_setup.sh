...
function gitRetryCommand() {
  ...
  for retries in $(seq 1 $(echo "${1}")); do
    # Run the command ${3} parameter.  If it succeeds, return from function.  Otherwise echo failed and then retry ${1} number of times
    RESULT=$(eval ${3} > /dev/null 2>&1) && return ${RESULT})
    [[ "${retries}" > 1 ]] && outputMode "Trying again in ${2} seconds"
    sleep ${2}
    # Exit out completely if we've failed to run the command ${1} times
    # Error Code #400
...
# Create the Group in Gitlab
#curl -d "name=${CUSTOMER_ID}&path=${CUSTOMER_ID}&visibility=private&lfs_enabled=false" --header "PRIVATE-TOKEN: ${GIT_TOKEN}" https://${GIT_DOMAIN}/api/v4/groups
...
# Create the Admin User in Gitlab
COMMAND="curl \
     --request POST \
...
RETURNED_MESSAGE=$(gitRetryCommand "1" "3" "${COMMAND}")
COMMAND_RESPONSE=$?
if [[ "${COMMAND_RESPONSE}" == 0 ]]; then
  MESSAGE=$(echo "${RETURNED_MESSAGE}" | jq -r ".message")
  ID=$(echo "${RETURNED_MESSAGE}" | jq -r ".id")
else
  errorExit "Error code: 401"
fi


# Add Admin User as Member to the Project
curl --request POST  --header "PRIVATE-TOKEN: ${GIT_TOKEN}" \
         --data "user_id=${SOMETHING}" \
         --data "access_level=123" \
     https://${GIT_DOMAIN}/api/v4/projects/${THE id OF user}/members

# Test against Groups
curl --request POST  --header "PRIVATE-TOKEN: ${GIT_TOKEN}" \
         --data "user_id=${SOMETHING}" \
         --data "access_level=123" \
     https://${GIT_DOMAIN}/api/v4/groups/${THE id OF user}/members
